...
# yazi change directory context after search
function y() {
...
# Load aliases
if [ -f /home/tbird/.bash_aliases ]; then
  . /home/tbird/.bash_aliases
fi
...
# Load WINE and WINETRICKS locations as ENV variables
export WINE="/usr/bin/wine"
export WINETRICKS="/usr/bin/winetricks"

#if [ -x "$(command -v tmux)" ] && [ -n "${DISPLAY}" ] && [ -z "${TMUX}" ] && [ "${TERM}" != "xterm-256color" ]; then
